...
###################################################################################################
=head2 shortest_path_parents - A variation of Dijkstra's single-source shortest paths algorithm to determine all the possible parents of each node that are in the shortest paths between that node and the given source.
...
sub shortest_path_parents {
    ...
    # Get path costs
    my $dbh = $class->db_Main;
    my $q = $dbh->selectall_arrayref("SELECT device.id, 
